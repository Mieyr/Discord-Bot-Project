...
try:
    mydb = mysql.connector.connect(
        ...
        user = 'INSERT USERNAME', # Redact
        password = 'INSERT PASSWORD', # Redact
        database = 'INSERT DATABASE' # Redact
    )
...
def Update_User(New_Balance, author_id, Coin_Booster):
    Sql = "UPDATE user_stats SET Balance = %s, Coin_Booster = %s WHERE User_ID = %s"
    Data_Tuple = (New_Balance, Coin_Booster, author_id)
...
def Fetch_Data_User(author_id):
    ...
    for row in results:
        Balance = row['Balance']
        Coin_Booster = row['Coin_Booster']
    return int(Balance), results, Coin_Booster
